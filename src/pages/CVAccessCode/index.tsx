...
function CV() {
  ...
  const { data: work_experiences_api_response_data } =
    useGetWorkExperiencesQuery(userData?.admin_account_id);
  const { data: skills_api_response_data } = useGetSkillsQuery(
    userData?.admin_account_id
  );
  const { data: references_api_response_data } = useGetReferencesQuery(
    userData?.admin_account_id
  );
  const { data: educations_api_response_data } = useGetEducationsQuery(
    userData?.admin_account_id
  );
  ...
  const work_experiences = work_experiences_api_response_data
    ? work_experiences_api_response_data?.data.map((ex: any) => ex?.attributes)
    : [];
  const skills = skills_api_response_data
    ? skills_api_response_data?.data.map((skill: any) => skill?.attributes)
    : [];
  const references = references_api_response_data
    ? references_api_response_data?.data.map((rf: any) => rf?.attributes)
    : [];
  const educations = educations_api_response_data
    ? educations_api_response_data?.data.map((ed: any) => ed?.attributes)
    : [];
...
function Main() {
  ...
  const { data: users } = useGetUsersDataQuery(null);
  const bearerToken = useAppSelector((state) => {
    return state.user?.bearerToken;
  });
  const cv_access_code = useAppSelector((state) => state.user?.cv_access_code);
  ...
  useEffect(() => {
    let access_code_user_data = users?.find((user: any) => {
      return user?.cv_access_code == cv_access_code;
    });

    if (!access_code_user_data) {
      if (
        confirm(
          "Invalid Access code. Please request the valid access code from the CV owner"
        ) == true
      ) {
    ...
    setUserData(access_code_user_data);
  }, [cv_access_code]);
